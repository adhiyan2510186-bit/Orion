"""Parquet-backed DataProvider over the committed ARGO fixture.

The v0 default. `IMPLEMENTATION_PLAN.md` originally named a CSV provider; the same
data is 159 MB as CSV against ~6 MB as zstd Parquet, so CSV was never a viable
committed fixture. See docs/adr/0001-parquet-over-csv.md.

Capabilities are declared honestly: this provider filters and aggregates in-process
over a pandas frame, so it claims SERVER_SIDE_FILTERING and AGGREGATION because it
genuinely applies the whole spec itself, and nothing else.
"""
...
DEFAULT_FIXTURE = (
    Path(__file__).resolve().parents[2] / "data" / "samples" / "argo_equatorial_pacific.parquet"
)
    ...
    async def initialize(self) -> None:
        ...
        if not self._path.exists():
            raise FileNotFoundError(
                f"Fixture missing: {self._path}. Build it with '.\\make.ps1 seed' "
                "(requires backend/data/raw/*.nc, fetched by '.\\make.ps1 fetch-data')."
            )
        frame = await asyncio.to_thread(pd.read_parquet, self._path)
...
__all__ = ["ACCEPTABLE_QC", "AnomalyTag", "ParquetProvider", "ProfileDerived"]
